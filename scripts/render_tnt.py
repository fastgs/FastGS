...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, args, background, 
    ...
    render_mesh_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_mesh")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        gray = np.linalg.norm(normal, axis=-1)
        ...
        cv2.imwrite(os.path.join(render_mesh_path, view.image_name + ".jpg"), gray)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, args, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians, pipeline, args, background, 
        ...
        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), scene, gaussians, pipeline, args, background)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args, args.skip_train, args.skip_test, args.max_depth, args.voxel_size, args.num_cluster, args.use_depth_filter)